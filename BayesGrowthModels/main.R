...
p = res_plot(res) + ggtitle('Logistic Growth Model')
ggsave(filename = 'logistic_growth_model.png', plot = p, width = 9, height = 6)

# Fit the generalized logistic growth model
res <- growth_model_grm(cumulative_cases, -1.0, 1.0, N, T_forecast, save);
p = res_plot(res) + ggtitle('Generalized Logistic Growth Model')
ggsave(filename = 'generalized_logistic_growth_model.png', plot = p, width = 9, height = 6)

# Fit the Richards growth model
res <- growth_model_grm(cumulative_cases, 1.0, -1.0, N, T_forecast, save);
p = res_plot(res) + ggtitle('Richards Growth Model')
ggsave(filename = 'richards_growth_model.png', plot = p, width = 9, height = 6)

# Fit the generalized Richards growth model
res <- growth_model_grm(cumulative_cases, -1.0, -1.0, N, T_forecast, save);
p = res_plot(res) + ggtitle('Generalized Richards Growth Model')
ggsave(filename = 'generalized_richards_growth_model.png', plot = p, width = 9, height = 6)

# Fit the Bertalanffy growth model
res <- growth_model_grm(cumulative_cases, 2/3, 1/3, N, T_forecast, save);
p = res_plot(res) + ggtitle('Bertalanffy growth model')
ggsave(filename = 'bertalanffy_growth_model.png', plot = p, width = 9, height = 6)

# Fit the Gompertz growth model
res <- growth_model_2p(cumulative_cases, N, T_forecast, 3, save);
p = res_plot(res) + ggtitle('Gompertz growth model')
ggsave(filename = 'gompertz_growth_model.png', plot = p, width = 9, height = 6)
